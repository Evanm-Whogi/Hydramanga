...
import { reportUser } from "@/services/profileService";
...
export default function ReportUserModal({ identifier, username, onClose }: ReportUserModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await reportUser(identifier, { username, reportType, details: details.trim() || undefined });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={onClose}>
      <div className="bg-background border border-borders rounded-lg shadow-xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-borders px-4 py-3">
          <h2 className="text-lg font-semibold flex items-center gap-2"><Flag className="size-5 text-muted" /> Report User</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          <p className="text-sm text-muted">Report &quot;{username}&quot; if they violate community guidelines.</p>
