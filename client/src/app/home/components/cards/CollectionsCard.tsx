...
export default function CollectionCard({ collection }: { collection: any }) {
    return (
        <Link href={`/discover?genres=${collection.name}`} className="relative aspect-2/3 w-full overflow-hidden rounded-2xl">
            ...
            <div className="absolute bottom-0 inset-x-0 flex flex-col items-start p-3 bg-linear-to-t from-transparent-card/80 to-transparent-card/50">
                <h3 className="text-lg font-extrabold text-primary leading-tight line-clamp-2">{collection.name}</h3>
