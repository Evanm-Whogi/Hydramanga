...
export default function SearchBar({ onChange, size, initialValue, placeholder = "Search manga or authors..." }: { onChange: (val: string) => void, size?: string, initialValue?: string, placeholder?: string }) {
    ...
    return (
        <div>
            <div className="group block relative">
                ...
                <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={placeholder} className={`bg-foreground border border-borders text-primary rounded-md px-3 py-1 pl-9 outline-none transition-all focus:ring-2 focus:ring-accent ${size || "w-64"}`} />
            </div>
        </div>
    );
}