            ...
            {() => (
                <div className="space-y-2 min-w-0 overflow-hidden">
                    <div className="relative min-w-0">
                        ...
                        <input type="text" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tags..." className="w-full min-w-0 bg-background border border-borders text-primary rounded-md px-3 py-1 pl-8 text-sm outline-none transition-all focus:ring-2 focus:ring-accent"/>
                    </div>
                    <div className="max-h-44 overflow-y-auto overflow-x-hidden min-w-0">
                        ...
                        {filteredOptions.map((tag) => (
                            <label key={tag} className="flex items-center gap-3 px-2 py-1.5 rounded hover:bg-white/5 cursor-pointer group min-w-0">
                                ...
                                <span className="text-sm text-primary group-hover:text-gray-300 transition-colors min-w-0 truncate">{tag}</span>
                            </label>
                        ))}
                    </div>
                </div>
