/**
 * MangaFire VRF token generation for gated endpoints (/filter, /ajax/read/…).
 *
 * Ported from the public MangaFire extension algorithm (RC4 + transform rounds).
 */
...
/** Generate a VRF token. Input is the raw keyword or endpoint-specific payload (e.g. `pjyy4@chapter@en`). */
export function generateMangaFireVrf(input: string): string {
    const schedule0 = [
