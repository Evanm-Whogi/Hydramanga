...
import { appendCatalogTitle, scraperTitleOptions } from '@/lib/catalogTitles';
...
export async function adminScraperSearch(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
        ...
        const mangaName = queryOverride || resolveDisplayTitle(row);
...
export async function adminAddSecondaryTitle(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
        ...
        const updatedTitles = appendCatalogTitle(row.titles, { title: title.trim(), language: lang, type: typeVal });
        ...
        return res.json({ success: true, seriesId: id, titles: updatedTitles });
    } catch (error) {
...
export async function adminUpdateSeries(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
        ...
        const allowed: (keyof typeof series.$inferSelect)[] = [
            'titles', 'description', 'note', 'status',
        ...
        for (const key of allowed) {
            if (key in body) {
                ...
                if (key === 'titles') {
                    if (val === null || Array.isArray(val)) updates[key] = val;
                    continue;
                }
